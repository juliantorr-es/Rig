...
def _write_fake_gemini(tmp_path: Path, name: str) -> Path:
    script = tmp_path / name
    _make_executable(
        script,
        """#!/bin/sh
set -eu
LOG_FILE="$RIG_FAKE_GEMINI_LOG"
printf '%s\n' "$*" >> "$LOG_FILE"
printf 'FAKE_GEMINI %s\n' "$*"
exit 0
""",
    )
    return script
...
def _run_launcher(*args: str, env: dict[str, str] | None = None):
    ...
    return __import__("subprocess").run(
        [sys.executable, "scripts/rig_gemini_launcher.py", *args],
...
def test_doctor_reports_guard_active(tmp_path):
    fake_gemini = _write_fake_gemini(tmp_path, "gemini")
    env = {
        "PATH": _minimal_path(tmp_path, "/Users/user/Developer/GitHub/Rig/scripts"),
        "RIG_FAKE_GEMINI_LOG": str(tmp_path / "gemini.log"),
        "RIG_REAL_GEMINI": str(fake_gemini),
    }
...
def test_launcher_fails_when_guard_inactive(tmp_path):
    ...
    assert "Blocked by Rig Gemini launcher" in result.stderr


def test_launcher_honors_rig_real_gemini(tmp_path):
    fake_gemini = _write_fake_gemini(tmp_path, "custom-gemini")
    env = {
        "PATH": _minimal_path(tmp_path, "/Users/user/Developer/GitHub/Rig/scripts"),
        "RIG_FAKE_GEMINI_LOG": str(tmp_path / "gemini.log"),
        "RIG_REAL_GEMINI": str(fake_gemini),
    ...
    assert result.returncode == 0
    assert "FAKE_GEMINI --agent alpha" in result.stdout


def test_launcher_discovers_gemini_on_path(tmp_path):
    _write_fake_gemini(tmp_path, "gemini")
    env = {
        "PATH": _minimal_path(tmp_path, "/Users/user/Developer/GitHub/Rig/scripts"),
        "RIG_FAKE_GEMINI_LOG": str(tmp_path / "gemini.log"),
    }
    ...
    assert result.returncode == 0
    assert "FAKE_GEMINI --agent beta" in result.stdout


def test_launcher_discovers_mistral_gemini_when_gemini_absent(tmp_path):
    _write_fake_gemini(tmp_path, "mistral-gemini")
    env = {
        "PATH": _minimal_path(tmp_path, "/Users/user/Developer/GitHub/Rig/scripts"),
        "RIG_FAKE_GEMINI_LOG": str(tmp_path / "gemini.log"),
    }
    ...
    assert result.returncode == 0
    assert "FAKE_GEMINI --agent gamma" in result.stdout


def test_launcher_avoids_selecting_itself_recursively(tmp_path):
    env = {
        "PATH": _minimal_path("/Users/user/Developer/GitHub/Rig/scripts", tmp_path),
        "RIG_REAL_GEMINI": str(tmp_path / "missing"),
    }
    result = _run_launcher("--doctor", env=env)
    assert result.returncode in (0, 1)
    assert "discovered_gemini: NOT_FOUND" in result.stdout or "discovered_gemini:" in result.stdout
...
def test_launcher_preserves_and_forwards_arguments(tmp_path):
    ...
    assert result.returncode == 0
    assert "FAKE_GEMINI --agent delta --mode safe" in result.stdout
...
def test_launcher_fails_when_no_gemini_found(tmp_path):
    env = {
        "PATH": _minimal_path(tmp_path, "/Users/user/Developer/GitHub/Rig/scripts"),
    }
    result = _run_launcher("--agent", "epsilon", env=env)
    assert result.returncode == 127
    assert "no Gemini executable found" in result.stderr
