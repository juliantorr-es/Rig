...
class WorkspaceGovernance:
    ...
    def load_workspace(self, workspace_id: str) -> Optional[WorkspaceRecord]:
        ...
        if not path.exists():
            return None
        return WorkspaceRecord(load_json(path), path)
    ...
    def list_workspaces(self) -> list[dict[str, Any]]:
        return [load_json(path) for path in sorted(self.workspace_dir.glob("*.json"))]
    ...
    def build_review_bundle(self, workspace_id: str) -> dict[str, Any]:
        ...
        validation = load_json(validation_path) if validation_path.exists() else self.generate_validation_result(workspace_id)
    ...
    def apply_workspace(self, workspace_id: str) -> dict[str, Any]:
        ...
        validation = load_json(self.validation_result_path(workspace_id))
