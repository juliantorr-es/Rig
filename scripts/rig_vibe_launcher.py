...
COMMON_VIBE_NAMES = ("vibe", "mistral-vibe")
...
def _discover_vibe(env: dict[str, str]) -> str | None:
    override = env.get("RIG_REAL_VIBE")
    if override:
...
def _doctor(env: dict[str, str]) -> int:
    ...
    vibe_path = _discover_vibe(launched_env)
    ...
    print(f"RIG_REAL_VIBE: {'set' if env.get('RIG_REAL_VIBE') else 'unset'}")
    print(f"discovered_vibe: {vibe_path or 'NOT_FOUND'}")
    print("path_head:")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not git_path:
        print(
            "Blocked by Rig Vibe launcher: guarded git is not active.\n"
            f"Expected `which git` to resolve to {GUARD_GIT}.\n"
            "Set PATH so the Rig Git guard comes first before launching Vibe.",
            file=sys.stderr,
        )
        return 126

    vibe_path = _discover_vibe(env)
    if not vibe_path:
        print(
            "Blocked by Rig Vibe launcher: no Vibe executable found.\n"
            "Set RIG_REAL_VIBE or install `vibe`/`mistral-vibe` on PATH.",
            file=sys.stderr,
        )
        return 127

    result = subprocess.run([vibe_path, *argv], env=env, check=False)
